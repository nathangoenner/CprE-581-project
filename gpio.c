...
#define LED 0x03   /* Assumes bit 0 of GPIO is connected to an LED  */
...
{
    u32 reg_val = XGpio_DiscreteRead(&Gpio, LED_CHANNEL);
    if (value)
    {
        reg_val |= 0x1;
        XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, reg_val);
    }
    else {
        reg_val &= ~0x1;
        XGpio_DiscreteWrite(&Gpio, LED_CHANNEL, reg_val);
